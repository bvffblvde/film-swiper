    ...
    <>
      ...
      >
        <Box style={{ padding: '20px 24px', borderBottom: '1px solid #2C2E33', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <Button variant="subtle" color="gray" size="xs" leftSection={<XIcon size={14} weight="bold" />} onClick={onClose}>
            Закрыть
          </Button>
        </Box>
        ...
        <Box style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
          {matches.length === 0 ? (
            ...
            >
              {matches.map((movie, i) => (
                <Box key={movie.id} onClick={() => setDetailMovie(movie)} style={{ cursor: 'pointer' }}>
                  <Box style={{
                    position: 'relative', width: '100%', paddingBottom: '150%', height: 0,
                    overflow: 'hidden', borderRadius: '12px', background: '#25262b',
                  }}>
                    <Box style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}>
                      ...
                      <Badge style={{ position: 'absolute', top: '8px', left: '8px', background: 'rgba(250,176,5,0.9)', color: '#fff', fontSize: '12px', fontWeight: 800, padding: '3px 8px' }}>
                        #{i + 1}
                      </Badge>
        ...
        {isHost && (
          <Box style={{ padding: '16px 24px', borderTop: '1px solid #2C2E33', flexShrink: 0 }}>
            <Button fullWidth size="md" color="red" variant="light" leftSection={<XIcon size={16} weight="bold" />} onClick={onFinish} style={{ maxWidth: '420px', margin: '0 auto', display: 'block' }}>
              Завершить сессию
            </Button>
          </Box>
        )}
