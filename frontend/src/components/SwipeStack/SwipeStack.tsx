    ...
    >
      {isTop && (
        <motion.div
          style={{
            ...
            border: '3px solid #51CF66', borderRadius: '8px', padding: '4px 12px',
            pointerEvents: 'none',
          }}
        >
          <Text fw={900} style={{ fontSize: '22px', color: '#51CF66', letterSpacing: '1px' }}>
            <HeartIcon size={20} weight="fill" style={{ marginRight: '4px' }} />ЛАЙК
          </Text>
        </motion.div>
      )}
      ...
      {isTop && (
        <motion.div
          style={{
            ...
            border: '3px solid #FF6B6B', borderRadius: '8px', padding: '4px 12px',
            pointerEvents: 'none',
          }}
        >
          <Text fw={900} style={{ fontSize: '22px', color: '#FF6B6B', letterSpacing: '1px' }}>
            <XIcon size={20} weight="bold" style={{ marginRight: '4px' }} />НОПЕ
          </Text>
        </motion.div>
      )}
      ...
      {isTop && onInfo && (
        <Box
          ...
          style={{
            position: 'absolute', bottom: '48%', right: '12px', zIndex: 20,
  ...
  return (
    <Box style={{ position: 'relative', width: '100%', height: '100%' }}>
        ...
        {visibleMovies.map((movie, i) => {
          ...
          return (
            <motion.div
              ...
              exit={
                isLeaving
                  ? { x: leaving.direction === 'right' ? 500 : -500, opacity: 0, transition: { duration: 0.3 } }
                  : {}
              }
            >
              <DraggableCard
      ...
      {visibleMovies.length === 0 && (
        <Stack align="center" justify="center" gap={12} style={{ height: '100%' }}>
