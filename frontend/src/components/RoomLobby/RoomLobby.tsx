...
export function RoomLobby({
  ...
  const participantsList = (
    <Box style={{ background: '#1A1B1E', border: '1px solid #2C2E33', borderRadius: '16px', padding: '20px', height: 'auto' }}>
  ...
  const classicSettingsPanel = isHost ? (
    <Box style={{ background: '#1A1B1E', border: '1px solid #2C2E33', borderRadius: '16px', padding: '20px' }}>
      ...
      <Stack gap={0}>
        ...
        {/* Year range */}
        <Box pb={20}>
          <Text size="xs" c="dimmed" fw={500} mb={10}>Год выпуска</Text>
          <Group gap={8} align="center">
            <NumberInput
              ...
              onChange={(v) => handleChange('yearFrom', Number(v) || 0)}
              style={{ flex: 1 }}
              styles={inputStyles}
            />
            ...
            <NumberInput
              ...
              onChange={(v) => handleChange('yearTo', Number(v) || 0)}
              style={{ flex: 1 }}
              styles={inputStyles}
            />
          </Group>
        ...
        {/* Min rating */}
        <Box pb={24}>
          <Text size="xs" c="dimmed" fw={500} mb={10}>
            Рейтинг от {settings.minRating > 0 ? settings.minRating.toFixed(1) : '—'}
          </Text>
          <Slider
        ...
        {/* Required matches */}
        <Box pb={20}>
          <Text size="xs" c="dimmed" fw={500} mb={8}>Матчей до конца сессии</Text>
          <NumberInput
            min={1} max={20}
        ...
        {/* Country exclusion */}
        <Box>
          <Text size="xs" c="dimmed" fw={500} mb={8}>Исключить страны</Text>
          <MultiSelect
            data={COUNTRY_OPTIONS}
            value={settings.excludedCountries}
            onChange={(v) => handleChange('excludedCountries', v)}
            placeholder="Все страны включены"
            searchable
            clearable
            ...
            styles={{
              input: { background: '#25262b', border: '1px solid #373A40', color: '#fff' },
              dropdown: { background: '#25262b', border: '1px solid #373A40' },
              pill: { background: '#373A40', color: '#C1C2C5' },
            }}
          />
        </Box>
      </Stack>
